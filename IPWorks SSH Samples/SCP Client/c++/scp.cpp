...
int main(int argc, char **argv)
{
	...
	if (argc >= 5)
	{
		...
		scp.SetSSHUser(argv[3]);
		scp.SetSSHPassword(argv[4]);
		sexec.SetSSHUser(argv[3]);
		sexec.SetSSHPassword(argv[4]);
		ret_code = scp.SSHLogon(argv[1], port);
		ret_code2 = sexec.SSHLogon(argv[1], port);
	}
	else
	{
		printf ("SSH Server: " );
		...
		ret_code = scp.SSHLogon(scp.GetSSHHost(), port);
		ret_code2 = sexec.SSHLogon(scp.GetSSHHost(), port);
	}
	...
	while (1)
	{
		...
		else if ( ! strcmp(command, "get") )
		{
			...
			{
				printf("Invalid command!\n\nThe expected format is \"get <RemoteFile> <LocalFile>\"\n");
			}
			else
			{
		...
		else if ( ! strcmp(command, "put") )
		{
			...
			if (!argument || !argument2)
			{
				printf("Invalid command!\n\nThe expected format is \"put <LocalFile> <RemoteFile>\"\n");
			}
			else
			{
